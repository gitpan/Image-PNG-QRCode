...
static unsigned char ECCLEVEL;
static unsigned char neccblk1;
static unsigned char neccblk2;
static unsigned char datablkw;
static unsigned char eccblkwid;
...
static void stringtoqr (qr_t * qr)
{
    ...
    initrspoly(eccblkwid,qr->qrframe);

    for (i = 0; i < neccblk1; i++) {
        appendrs(dat, datablkw, ecc, eccblkwid, qr->qrframe);
        dat += datablkw;
        ecc += eccblkwid;
    }
    for (i = 0; i < neccblk2; i++) {
        appendrs(dat, datablkw + 1, ecc, eccblkwid, qr->qrframe);
        dat += datablkw + 1;
        ecc += eccblkwid;
    }
    unsigned j;
    ...
    for (i = 0; i < datablkw; i++) {
        for (j = 0; j < neccblk1; j++)
            *dat++ = qr->strinbuf[i + j * datablkw];
        for (j = 0; j < neccblk2; j++)
            *dat++ = qr->strinbuf[(neccblk1 * datablkw) + i + (j * (datablkw + 1))];
    }
    for (j = 0; j < neccblk2; j++)
        *dat++ = qr->strinbuf[(neccblk1 * datablkw) + i + (j * (datablkw + 1))];
    for (i = 0; i < eccblkwid; i++)
        for (j = 0; j < neccblk1 + neccblk2; j++)
            *dat++ = qr->strinbuf[max + i + j * eccblkwid];
    memcpy(qr->strinbuf, qr->qrframe, max + eccblkwid * (neccblk1 + neccblk2));
